...
class EliteSniperV2:
    ...
    def _submit_form(self, page: Page, session: SessionState, worker_logger) -> bool:
        ...
        worker_logger.info("[SUBMIT] Starting natural submission...")
        
        for attempt in range(1, max_attempts + 1):
            try:
                # Check if we're still on form page
                if not page.locator("input[name='lastname']").is_visible():
                    worker_logger.warning("[SUBMIT] Form disappeared")
                    return False
                ...
                if not success or not code:
                    worker_logger.warning(f"[SUBMIT] Captcha solve failed: {status}")
                    self.solver.reload_captcha(page)
                    time.sleep(1)
                    continue
                
                # Fill captcha naturally
                captcha_input = page.locator("input[name='captchaText']").first
                ...
                for selector in submit_selectors:
                    try:
                        ...
                        if btn.is_visible(timeout=1000):
                            btn.click(timeout=2000)
                            worker_logger.info(f"[SUBMIT] Clicked submit button: {selector}")
                            submitted = True
                            break
                    except:
                        continue
                ...
                if not submitted:
                    ...
                    page.keyboard.press("Enter")
                    worker_logger.info("[SUBMIT] Pressed Enter as fallback")
                ...
                # Silent reject - still on form
                if page.locator("input[name='lastname']").is_visible():
                    worker_logger.info(f"[SILENT] Silent reject - attempt {attempt}")
                    self.solver.reload_captcha(page)
                    time.sleep(2)
                    continue
                ...
                if "beginnen sie den buchungsvorgang neu" in content:
                    worker_logger.error("[HARD_FAIL] Session expired")
                    return False
                    
            except Exception as e:
                worker_logger.error(f"[SUBMIT] Error in attempt {attempt}: {e}")
                time.sleep(2)
        
        worker_logger.warning(f"[SUBMIT] Max attempts ({max_attempts}) reached")
        return False
