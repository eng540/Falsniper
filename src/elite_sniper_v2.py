...
class EliteSniperV2:
            ...
            for cycle in range(max_cycles):
                ...
                for i, url in enumerate(month_urls):
                    ...
                    if self._submit_form(page, session, worker_logger):
                        ...
                        # Notification handled in _submit_form and _handle_success
                        # try:
    ...
    def _submit_form(self, page: Page, session: SessionState, worker_logger) -> bool:
        """
        [ULTIMATE PATCH V2] - Fixed Selectors & Navigation Logic
        Handles: Soft Fails, Hard Fails, and Race Conditions with proper Playwright syntax.
        """
        worker_id = session.worker_id
        max_attempts = 15
        worker_logger.info(f"🚀 STARTING FINAL SUBMISSION SEQUENCE...")
            ...
            try:
                ...
                if not captcha_input.is_visible():
                    ...
                    if "beginnen sie den buchungsvorgang neu" in content or "ref-id:" in content:
                        ...
                        return False
                    worker_logger.warning("⚠️ Form not found. Possible navigation in progress...")
                    time.sleep(2)
                    continue
                ...
                # Use expect_navigation to handle .do full page reloads
                try:
                ...
                # 4. Analyze Result with Correct Playwright Syntax
                # We check multiple conditions to determine the state
                time.sleep(1) # Final stability wait
                content = page.content().lower()
                ...
                # Case B: Hard Fail (Session Poisoning)
                if "beginnen sie den buchungsvorgang neu" in content or "ref-id:" in content:
                ...
                worker_logger.error("❓ Unknown page state. Saving debug info...")
                self.debug_manager.save_critical_screenshot(page, "UNKNOWN_STATE", worker_id)
